...
class KeyMonotonicityValidationSpec
  ...
  private val baseTime = ZonedDateTime.of(2021, 1, 1, 12, 0, 0, 0, ZoneOffset.UTC).toInstant
  private val testTransactionEntry = {
    DamlTransactionEntrySummary(
      DamlTransactionEntry
        .newBuilder()
        .setSubmissionSeed(testSubmissionSeed)
        .setLedgerEffectiveTime(Conversions.buildTimestamp(baseTime))
        .build()
    )
  }
  ...
  "checkContractKeysCausalMonotonicity" should {
    "create StepContinue in case of correct keys" in {
      KeyMonotonicityValidation.checkContractKeysCausalMonotonicity(
        ...
        None,
        Set(testKey),
        Map(testKey -> buildStateValue(baseTime.minusSeconds(1))),
    ...
    "reject transaction in case of incorrect keys" in {
      ...
      KeyMonotonicityValidation
        .checkContractKeysCausalMonotonicity(
          ...
          None,
          Set(testKey),
          Map(testKey -> buildStateValue(baseTime.plusSeconds(1))),
          testTransactionEntry,
        )
  ...
  private def buildStateValue(activeAt: Instant) = {
    DamlStateValue
      .newBuilder()
      .setContractKeyState(
